 ...
 *
 * 3 min keeps DB cost low; the strip can trail real status by up to that
 * long, which is fine — the Discord board only refreshes every 5 min, and
 * the /status page is a periodic health glance, not a live feed. Cache key
 * includes both kwargs
