...
/**
 * Per-isolate guard so the cross-instance DB claim (claimThrottleWindow — an
 * INSERT…ON CONFLICT) doesn't fire on every chat completion.
 * refreshStatusBoard runs fire-and-forget after every /v1 request; the claim
 * only WINS once per THROTTLE_MS, but every losing attempt was still a DB
 * round-trip. If this isolate already attempted within the window, we return
 * before touching the DB. The DB claim stays the authority for whichever
 * isolate does get through, so cross-instance serialization is unchanged —
 * each warm isolate just stops re-attempting (and re-writing) on every chat.
 */
...
export async function refreshStatusBoard(): Promise<void> {
  try {
    ...
    // Per-isolate pre-filter: if THIS isolate attempted a claim within the
    // window, skip before any DB round-trip. Set on every attempt (win or
    // lose) so a warm isolate fires the claim at most once per window.
    const claimNow = Date.now();
    if (claimNow - lastClaimAttemptAt < THROTTLE_MS) return;
    lastClaimAttemptAt = claimNow;
