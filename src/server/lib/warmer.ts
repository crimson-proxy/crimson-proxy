 ...
 *                    request_logs traffic, AND age out request_logs
...
/**
 * request_logs retention. Nothing reads request_logs beyond a 24h window
 * (the usage limiter in lib/usage-limit.ts), so rows older than this are only
 * dashboard backscroll — safe to delete, and it keeps the table and its
 * indexes from growing without bound. Tune RETENTION_DAYS freely.
 */
const RETENTION_DAYS = 30;
/** Cap rows deleted per run so a large first-time backlog drains over a few
 *  runs instead of one long delete on a serverless background task. */
 ...
 *
 * Also enforces request_logs retention (RETENTION_DAYS) in the same pass,
 * batched so a backlog never becomes one long delete.
 */
export async function maybePrune(): Promise<void> {
  try {
    ...
    // request_logs retention, bounded per run. Best-effort: a failure here
    // must not block the throttle bump below — the next pruner tick just
    // retries the delete and chips away at the backlog.
    try {
      await sql`
      ...
      console.error("[warmer] request_logs retention failed:", (err as Error).message);
    }
