...
auth.get("/api/me/logs", async (c) => {
  ...
  try {
    ...
    // page), computed in SQL so we never pull the user's entire history into
    // the function just to sum it. Deliberately ignores statusFilter so the
    // success-rate stays meaningful.
    const [agg] = await sql<{
      requests: number;
      successful: number;
      errors: number;
      ...
      select
        ...
        count(*) filter (where status < 200 or status >= 400)::int as errors,
        coalesce(sum(prompt_tokens), 0)::bigint as prompt_tokens,
        coalesce(sum(completion_tokens), 0)::bigint as completion_tokens,
        coalesce(sum(total_tokens), 0)::bigint as total_tokens,
        coalesce(round(sum(coalesce(duration_ms, 0))::numeric / nullif(count(*), 0)), 0)::int as avg_duration
      from request_logs
      where discord_user_id = ${user.sub} ${sinceCond} ${modelCond}
    `;
    const requests = Number(agg.requests);
    const successful = Number(agg.successful);
    const summary = {
      requests,
      successful,
      errors: Number(agg.errors),
      successRate:
        requests > 0 ? Math.round((successful / requests) * 1000) / 10 : 0,
      promptTokens: Number(agg.prompt_tokens),
      completionTokens: Number(agg.completion_tokens),
      totalTokens: Number(agg.total_tokens),
      avgDurationMs: Number(agg.avg_duration),
    };
